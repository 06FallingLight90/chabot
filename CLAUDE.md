...
- **调度（低后台占用）**：单条链式 `setTimeout`（非轮询，空闲零唤醒），`_dueAt` 记录下次触发时刻；**退后台不清定时器**——单条 pending timeout 后台零 CPU，H5 隐藏标签页被浏览器节流后仍会触发，App 后台 JS 挂起不触发时由回前台 `catchUpProactive()` 检测到期补发；App/聊天页 `onShow`、切换会话、保存设置后调用 `catchUpProactive()` 让调度立即按最新会话设置重排；**改设置/切会话用 `rearmProactive()` 强制重算**（`catchUp` 在定时器已挂载时跳过重排以保持跨页倒计时不重置，故"修改倒计时值立即生效"须走 `rearmProactive`——设置页保存/调试按钮、聊天页新对话/切换会话均用后者）；**重排只看功能开关（`_featureOn`：开关+现实时间+API 配置），与 loading 抑制解耦**——抑制只拦截到期发送、不拦截重排，任何时序下调度器都不会失效（曾因抑制期间到期不重排导致"等待调度"、只能重启恢复）；`catchUpProactive` 补发时先清旧定时器（`_tick` 也以 `_dueAt` 非空为触发前提）防重复发送；`_nextDelay` 恒 ≥60s（时段外等窗口起点，含"当前分钟正好=起点"边界）防 0 延迟忙循环；**`_dueAt` 持久化到 storage（`chabot_setting_proactive_due_at`）**——进程结束（关页/杀进程）重开时内存 `_dueAt` 清零，`catchUpProactive` 从持久化恢复：已过则补发一次再重排，仍在未来则按剩余时间重挂定时器（不重算档位间隔）；关闭拟真时清空持久化到期时刻防残留误触发；`getProactiveCountdown()` 返回下次触发倒计时（毫秒，未调度返回 null）供设置页实时显示；每次重排写一条 `拟真聊天调度` info 日志（含倒计时与档位）
- **触发门禁**（`sendProactiveBurst`，非调试）：开关 + 现实时间 + 未抑制（聊天页 loading 时 `setProactiveSuppressed(true)` 抑制）+ 时段窗口内 + 会话已有用户消息；**每次被门禁拦截都会写一条「主动消息跳过」info 日志并附原因**（如"不在时段窗口/无用户消息/仍在上一条/未开启"），避免"只见调度、不知为何不发"；**自定义倒计时（调试值）会忽略时段窗口与"已有用户消息"两个门禁**，便于随时验证发送链路
- **触发可观测（每次到期必出日志）**：`_tick`/`catchUpProactive` 到期调用 `sendProactiveBurst` 后，无论结果如何都保证有一条可见日志——成功→「主动消息已发送」（+ LLM 请求/响应）；被门禁拦截→「主动消息跳过」附原因，并额外写一条「拟真聊天到期·已到触发时刻但未发送」标记；**异常→「主动消息失败」err 日志并附完整堆栈**（替换原先 `.catch(()=>{})` 的静默吞错，杜绝"倒计时结束只见调度、无任何输出"）
