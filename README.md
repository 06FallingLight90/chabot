...
- **拟真聊天**：模拟真人发消息节奏——仅「现实时间」模式生效，AI 在随机时间节点主动给你发消息（每条 ≤1 句、连续表情 ≤2，多条消息分条发送）；可设置主动时段与频率档位（低/中/高），支持自定义倒计时（秒）便于调试；设置页实时显示下次触发倒计时、一键「立即发送一条主动消息」；前台单定时器调度 + 回前台补发，后台零轮询占用
